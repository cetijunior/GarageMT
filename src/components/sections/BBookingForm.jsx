...
const BookingForm = () => {
    ...
    // **Fetch booked time slots whenever date or garage changes**
    useEffect(() => {
        const fetchBookedTimes = async () => {
            if (!formData.date || !formData.garage) return; // Ensure both values exist before fetching

            const formattedDate = formData.date.toISOString().split("T")[0]; // Format date as YYYY-MM-DD

            try {
                const response = await axios.get(`https://garage-booking-backend.onrender.com/api/bookings/availability?date=${formattedDate}&garage=${formData.garage}`);
                setBookedTimes(response.data.bookedTimes || []); // Store booked times
            } catch (error) {
        ...
        fetchBookedTimes();
    }, [formData.date, formData.garage]); // Run when date or garage changes
